...
class XMSport(object):
    ...
    user_id: str
    app_token: str
    area: str = field(default="北京")
    headers: dict = field(default_factory=lambda x={
    ...
    # 登陆
    def login(self) -> bool:
        location_url = "https://api-user.huami.com/registrations/+86" + self.user + "/tokens"
        location_data = {
            "client_id": "HuaMi",
    ...
    def get_key_parameters(self) -> bool:
    ...
    # 运行主程序
    def main_run(self) -> dict:
        response = self.update_steps()
        if 'message' in response and response['message'] == 'success':
            state = {'state': response['message']}
        elif 'message' in response and response['message'] == 'invalid token':
            login_state = self.get_key_parameters()
            if not login_state:
                state = {
                    'state': 'login_error'
                }
